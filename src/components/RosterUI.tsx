...
/**
 * Roster UI — bubble layout (ES2019+)
...
function emptyWeek(positions: { id: string }[]) {
  const base: Record<string, Record<string, any[]>> = {};
  for (let i = 0; i < DAYS.length; i++) {
    const d = DAYS[i] as string;
    base[d] = {};
    for (let j = 0; j < positions.length; j++) base[d][positions[j].id] = [];
  }
  return base;
}

// Global dialog setter so Sortable children can open the editor
let editDialogSetState: ((s: any) => void) | null = null;
function openEditShiftDialog(args: any) {
  if (editDialogSetState) editDialogSetState(Object.assign({ open: true }, args));
}
...
export default function RosterUI() {
  ...
  const [positions, setPositions] = useState<{ id: string; name: string }[]>(saved.positions || defaultPositions);
  const [staff, setStaff] = useState<string[]>(saved.staff || defaultStaff);
  const [week, setWeek] = useState<Record<string, Record<string, any[]>>>(saved.week || emptyWeek(saved.positions || defaultPositions));
  const [activeDrag, setActiveDrag] = useState<any>(null);
  const [copiedDay, setCopiedDay] = useState<Record<string, any[]> | null>(null);
  ...
  useEffect(
    function () {
      saveState({ weekStart: weekStart, positions: positions, staff: staff, week: week });
    },
  ...
  function onDragEnd(event: any) {
    ...
    setActiveDrag(null);
    if (!over) return;
    const src = active && active.data && active.data.current;
    const dst = over && over.data && over.data.current;
    if (!src || !dst) return;
    setWeek(function (w) {
      const next = clone(w);
      const fromList = next[src.day] && next[src.day][src.positionId] ? next[src.day][src.positionId] : null;
      if (!fromList) return w;
      let idx = -1;
      for (let i = 0; i < fromList.length; i++) if (fromList[i].id === src.shiftId) {
        idx = i;
        break;
      }
      if (idx === -1) return w;
      const moved = fromList.splice(idx, 1)[0];
      if (!next[dst.day]) next[dst.day] = {} as any;
      ...
      if (dst.shiftId) {
        let insertAt = -1;
        for (let k = 0; k < toList.length; k++) if (toList[k].id === dst.shiftId) {
          insertAt = k;
          break;
        }
  ...
  function addPosition(name?: string) {
    ...
    setPositions(function (ps) {
      return ps.concat([p]);
    });
    setWeek(function (w) {
      const next = clone(w);
      for (let i = 0; i < DAYS.length; i++) next[DAYS[i] as string][p.id] = [];
      return next;
    });
  }
  ...
  function addShift(day: string, positionId: string, shift: any) {
    setWeek(function (w) {
      const n = clone(w);
      if (!n[day]) n[day] = {} as any;
      if (!n[day][positionId]) n[day][positionId] = [];
      n[day][positionId].push(Object.assign({ id: uid() }, shift));
      return n;
    });
  }
  function updateShift(day: string, positionId: string, shiftId: string, patch: any) {
    setWeek(function (w) {
      const next = clone(w);
      const arr = next[day] && next[day][positionId] ? next[day][positionId] : null;
      if (!arr) return w;
  ...
  function pasteDay(toDay: string) {
    if (!copiedDay) return;
    setWeek(function (w) {
      const n = clone(w);
        ...
        <table className="w-full min-w-[980px] text-sm border-separate border-spacing-x-0 border-spacing-y-2">
                  ...
                  <th key={d} className="p-0 text-center align-middle">
                    <div className="m-2 flex items-center justify-between gap-2 rounded-2xl bg-muted/40 px-3 py-2 shadow-sm">
                      ...
                      <div className="flex items-center gap-1">
                        <Button size="icon" variant="ghost" title="Copy day" onClick={function () { copyDay(d as string); }}><Copy className="w-4 h-4" /></Button>
                        <Button size="icon" variant="ghost" title="Paste day" onClick={function () { pasteDay(d as string); }} disabled={!copiedDay}><ClipboardPaste className="w-4 h-4" /></Button>
                      </div>
                    </div>
                  </th>
                );
          ...
          <tbody>
            <DndContext sensors={sensors} onDragStart={onDragStart} onDragEnd={onDragEnd} collisionDetection={closestCenter}>
              {positions.map(function (p, rowIndex) {
                return (
                            ...
                            onChange={function (e) {
                              ...
                              setPositions(function (ps) {
                                return ps.map(function (x) {
                                  return x.id === p.id ? { id: x.id, name: v } : x;
                                });
                              });
                            }}
                          />
                      ...
                      {DAYS.map(function (d) {
                        return (
                          <td key={d + "-" + p.id} className="align-top p-2">
                            <DayCell day={d as string} positionId={p.id} shifts={(week[d as string] && week[d as string][p.id]) ? week[d as string][p.id] : []} staff={staff} onAdd={addShift} onUpdate={updateShift} />
                          </td>
                        );
                      })}
                    </tr>
...
function DayCell({ day, positionId, shifts, staff, onAdd, onUpdate }: { day: string; positionId: string; shifts: any[]; staff: string[]; onAdd: (day: string, posId: string, payload: any) => void; onUpdate: (day: string, posId: string, shiftId: string, patch: any) => void; }) {
...
function SortableShift({ day, positionId, shift, onUpdate, staff }: { day: string; positionId: string; shift: any; onUpdate: (day: string, posId: string, shiftId: string, patch: any) => void; staff: string[]; }) {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({
    id: shift.id,
    data: { day: day, positionId: positionId, shiftId: shift.id, shift: shift },
  });
  const style = { transform: CSS.Transform.toString(transform), transition: transition as string | undefined } as React.CSSProperties;
  return (
...
function ShiftEditDialog() {
  ...
  return (
    <Dialog open={s.open} onOpenChange={function (v) { setState(function (prev: any) { return Object.assign({}, prev, { open: v }); }); }}>
      <DialogContent>
        <DialogHeader>
        ...
        <div className="grid sm:grid-cols-3 gap-3">
          <div>
            ...
            <Select value={s.shift.staff || ""} onValueChange={function (v) { s.onUpdate(s.day, s.positionId, s.shift.id, { staff: v }); setState(function (prev: any) { const nx = clone(prev); nx.shift.staff = v; return nx; }); }}>
              <SelectTrigger><SelectValue placeholder="Select staff" /></SelectTrigger>
              <SelectContent>
                {s.staff.map(function (name: string) { return <SelectItem key={name} value={name}>{name}</SelectItem>; })}
              </SelectContent>
            </Select>
          </div>
          <div>
            <div className="text-xs mb-1">Start</div>
            <Input type="time" value={s.shift.start || ""} onChange={function (e) { const v = e.target.value || null; s.onUpdate(s.day, s.positionId, s.shift.id, { start: v }); setState(function (prev: any) { const nx = clone(prev); nx.shift.start = v; return nx; }); }} />
          </div>
          <div>
            <div className="text-xs mb-1">End</div>
            <Input type="time" value={s.shift.end || ""} onChange={function (e) { const v = e.target.value || null; s.onUpdate(s.day, s.positionId, s.shift.id, { end: v }); setState(function (prev: any) { const nx = clone(prev); nx.shift.end = v; return nx; }); }} placeholder="Leave empty for Finish" />
          </div>
        </div>
      </DialogContent>
    </Dialog>
...
function ShiftForm({ staff, onSubmit }: { staff: string[]; onSubmit: (payload: any) => void }) {
...
function RosterSettings({ staff, setStaff, positions, addPosition, removePosition }: { staff: string[]; setStaff: React.Dispatch<React.SetStateAction<string[]>>; positions: { id: string; name: string }[]; addPosition: (name?: string) => void; removePosition: (id: string) => void; }) {
  const [open, setOpen] = useState(false);
                ...
                {staff.map(function (s, i) {
                  return (
                    <div key={i} className="flex items-center justify-between border rounded-xl p-2">
                      ...
                      <Button variant="ghost" size="icon" onClick={function () { setStaff(function (arr) { return arr.filter(function (_x, ix) { return ix !== i; }); }); }}>×</Button>
                    </div>
                  );
                })}
              </div>
...
function runRosterTests() {
  try {
    ...
    console.assert(DAYS.every(function (d) { const key = d as string; return wk[key] && Array.isArray(wk[key]["a"]) && Array.isArray(wk[key]["b"]); }), "emptyWeek: arrays for positions per day");

    // 2) addShift pushes an entry
    const w = emptyWeek([{ id: "p1" }]);
    const add = function (day: string, pid: string, shift: any) { if (!w[day]) (w as any)[day] = {}; if (!w[day][pid]) w[day][pid] = []; w[day][pid].push(Object.assign({ id: "x" }, shift)); };
    ...
    const copied = clone(w.Monday);
    const positions = [{ id: "p1" }, { id: "p2" }];
    const safe: Record<string, any[]> = {};
    positions.forEach(function (p) { safe[p.id] = (copied as any)[p.id] ? clone((copied as any)[p.id]) : []; });
    console.assert(Array.isArray(safe.p2), "pasteDay should init missing arrays");

    // 4) Finish display rule
    const isFinish = function (end: string | null) { return !end; };
    console.assert(isFinish(null as any) && isFinish("" as any) && !isFinish("17:00"), "Finish rule when end empty");
    ...
    console.warn("RosterUI tests error", e);
  }
}
