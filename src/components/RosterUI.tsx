...
import { DndContext, DragOverlay, PointerSensor, useSensor, useSensors, closestCenter, useDroppable, DragStartEvent, DragEndEvent } from "@dnd-kit/core";
...
/**
 * Roster UI — bubble layout (typed, no `any`)
 */
...
export interface Shift { id: string; staff?: string; start?: string | null; end?: string | null }
export type DayMap = Record<string, Shift[]>; // positionId -> shifts
export type Week = Record<Day, DayMap>;

interface DragData { day: Day; positionId: string; shiftId?: string; shift: Shift }
...
function loadState(): { weekStart?: string; positions?: Position[]; staff?: string[]; week?: Week } | null {
  try {
    const raw = typeof window !== "undefined" ? window.localStorage.getItem(LS_KEY) : null;
    return raw ? (JSON.parse(raw) as { weekStart?: string; positions?: Position[]; staff?: string[]; week?: Week }) : null;
  } catch {
    return null;
  }
}
...
let editDialogSetState: React.Dispatch<React.SetStateAction<EditDialogState>> | null = null;
function openEditShiftDialog(args: Omit<EditDialogState, "open"> & { open?: boolean }) {
  if (editDialogSetState) editDialogSetState((_) => ({ ...args, open: true } as EditDialogState));
}
...
export default function RosterUI() {
  ...
  const [positions, setPositions] = useState<Position[]>(saved.positions || defaultPositions);
  const [staff, setStaff] = useState<string[]>(saved.staff || defaultStaff);
  const [week, setWeek] = useState<Week>(saved.week || emptyWeek(saved.positions || defaultPositions));
  ...
  function addPosition(name?: string) {
    const p: Position = { id: uid(), name: name || `Position ${positions.length + 1}` };
  ...
  function updateShift(day: Day, positionId: string, shiftId: string, patch: Partial<Shift>) {
    setWeek((w) => {
      const next = clone(w);
  ...
  function clearWeek() {
    if (typeof window === "undefined" || window.confirm("Clear all shifts for this week?")) setWeek(emptyWeek(positions));
  }

  const dates = useMemo(() => (DAYS as readonly Day[]).map((_, i) => addDays(weekStart, i)), [weekStart]);
    ...
    <div className="p-4 sm:p-6 max-w-[1400px] mx-auto">
      <div className="flex items-center justify-between gap-3 mb-4">
        <div className="flex items-center gap-2">
          <Button variant="outline" onClick={() => setWeekStart(addDays(weekStart, -7))}><ChevronLeft className="w-4 h-4" /></Button>
          <WeekPicker value={weekStart} onChange={setWeekStart} />
          <Button variant="outline" onClick={() => setWeekStart(addDays(weekStart, 7))}><ChevronRight className="w-4 h-4" /></Button>
        </div>
        <div className="flex items-center gap-2">
                ...
                <th key={d} className="p-0 text-center align-middle">
                  <div className="m-2 flex items-center justify-between gap-2 rounded-2xl bg-muted/40 px-3 py-2 shadow-sm">
                    ...
                    <div className="flex items-center gap-1">
                      <Button size="icon" variant="ghost" title="Copy day" onClick={() => copyDay(d)}><Copy className="w-4 h-4" /></Button>
                      <Button size="icon" variant="ghost" title="Paste day" onClick={() => pasteDay(d)} disabled={!copiedDay}><ClipboardPaste className="w-4 h-4" /></Button>
                    </div>
                  </div>
                </th>
              ))}
                  ...
                  <tr className="group">
                      ...
                      <div className="relative inline-block text-center w-56 mx-auto">
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            ...
                            setPositions((ps) => ps.map((x) => (x.id === p.id ? { id: x.id, name: v } : x)));
                          }}
                        />
                      </div>
                    </td>
                    {(DAYS as readonly Day[]).map((d) => (
                      <td key={`${d}-${p.id}`} className="align-top p-2">
                        <DayCell day={d} positionId={p.id} shifts={week[d]?.[p.id] ?? []} staff={staff} onAdd={addShift} onUpdate={updateShift} />
                      </td>
                    ))}
                  </tr>
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="p-2">
        <Calendar mode="single" selected={value} onSelect={(d) => { if (d) onChange(startOfWeek(d, { weekStartsOn: 1 })); setOpen(false); }} initialFocus />
      </PopoverContent>
    </Popover>
  );
}
...
function AddPosition({ onAdd }: { onAdd: (name?: string) => void }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <Input placeholder="Add position" value={name} onChange={(e) => setName(e.target.value)} className="h-8 w-48" />
      <Button size="sm" onClick={() => { onAdd(name.trim()); setName(""); }}><Plus className="w-4 h-4 mr-1" />Add</Button>
    </div>
  );
}

function DayCell({ day, positionId, shifts, staff, onAdd, onUpdate }: { day: Day; positionId: string; shifts: Shift[]; staff: string[]; onAdd: (day: Day, posId: string, payload: Omit<Shift, "id">) => void; onUpdate: (day: Day, posId: string, shiftId: string, patch: Partial<Shift>) => void; }) {
  const { setNodeRef } = useDroppable({ id: `cell-${day}-${positionId}`, data: { day, positionId } });
  const [open, setOpen] = useState(false);
  return (
    <div ref={setNodeRef} className="space-y-2 min-h-[64px] rounded-xl p-1 hover:bg-muted/20 transition">
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent>
          ...
          <ShiftForm staff={staff} onSubmit={(payload) => { onAdd(day, positionId, payload); setOpen(false); }} />
        </DialogContent>
      </Dialog>
    </div>
  );
...
function SortableShift({ day, positionId, shift, onUpdate, staff }: { day: Day; positionId: string; shift: Shift; onUpdate: (day: Day, posId: string, shiftId: string, patch: Partial<Shift>) => void; staff: string[]; }) {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({
    id: shift.id,
    data: { day, positionId, shiftId: shift.id, shift } as DragData,
  });
  const style: React.CSSProperties = { transform: CSS.Transform.toString(transform), transition: transition as string | undefined };
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners}>
      <ShiftBubble staff={shift.staff} start={shift.start} end={shift.end} isDragging={isDragging} onClick={() => { openEditShiftDialog({ day, positionId, shift, onUpdate, staff }); }} />
    </div>
  );
}

function ShiftBubble({ staff, start, end, isDragging, onClick }: { staff?: string; start?: string | null; end?: string | null; isDragging?: boolean; onClick?: () => void; }) {
...
function ShiftEditDialog() {
  ...
  const s = state;
  return (
    <Dialog open={s.open} onOpenChange={(v) => setState((prev) => ({ ...prev, open: v }))}>
      <DialogContent>
        <DialogHeader>
        ...
        <div className="grid sm:grid-cols-3 gap-3">
          <div>
            ...
            <Select value={s.shift.staff || ""} onValueChange={(v) => { s.onUpdate(s.day, s.positionId, s.shift.id, { staff: v }); setState((prev) => ({ ...prev, shift: { ...prev.shift, staff: v } })); }}>
              <SelectTrigger><SelectValue placeholder="Select staff" /></SelectTrigger>
              <SelectContent>
                {s.staff.map((name) => (<SelectItem key={name} value={name}>{name}</SelectItem>))}
              </SelectContent>
            </Select>
          </div>
          <div>
            <div className="text-xs mb-1">Start</div>
            <Input type="time" value={s.shift.start || ""} onChange={(e) => { const v = e.target.value || null; s.onUpdate(s.day, s.positionId, s.shift.id, { start: v }); setState((prev) => ({ ...prev, shift: { ...prev.shift, start: v } })); }} />
          </div>
          <div>
            <div className="text-xs mb-1">End</div>
            <Input type="time" value={s.shift.end || ""} onChange={(e) => { const v = e.target.value || null; s.onUpdate(s.day, s.positionId, s.shift.id, { end: v }); setState((prev) => ({ ...prev, shift: { ...prev.shift, end: v } })); }} placeholder="Leave empty for Finish" />
          </div>
        </div>
      </DialogContent>
    </Dialog>
...
function ShiftForm({ staff, onSubmit }: { staff: string[]; onSubmit: (payload: Omit<Shift, "id">) => void }) {
  ...
  return (
    <div className="space-y-3">
      <div className="grid sm:grid-cols-3 gap-3">
        ...
        <div>
          <div className="text-xs mb-1">End</div>
          <Input type="time" value={end} onChange={(e) => setEnd(e.target.value)} placeholder="Leave empty for Finish" />
        </div>
      </div>
      <div className="flex justify-end gap-2">
        <Button onClick={() => { onSubmit({ staff: sel, start: start || null, end: end || null }); }}>Add</Button>
      </div>
    </div>
  );
}

function RosterSettings({ staff, setStaff, positions, addPosition, removePosition }: { staff: string[]; setStaff: React.Dispatch<React.SetStateAction<string[]>>; positions: Position[]; addPosition: (name?: string) => void; removePosition: (id: string) => void; }) {
  const [open, setOpen] = useState(false);
      ...
      <DialogContent className="max-w-2xl">
        ...
        <Tabs defaultValue="staff">
          ...
          <TabsContent value="staff" className="mt-4">
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                <Input placeholder="Add staff name" value={newStaff} onChange={(e) => setNewStaff(e.target.value)} className="h-8" />
                <Button size="sm" onClick={() => { if (newStaff.trim()) { setStaff((s) => s.concat([newStaff.trim()])); setNewStaff(""); } }}>Add</Button>
              </div>
              <div className="grid sm:grid-cols-2 gap-2">
                {staff.map((s, i) => (
                  <div key={i} className="flex items-center justify-between border rounded-xl p-2">
                    <Input className="h-8" value={s} onChange={(e) => { setStaff((arr) => arr.map((x, ix) => (ix === i ? e.target.value : x))); }} />
                    <Button variant="ghost" size="icon" onClick={() => { setStaff((arr) => arr.filter((_, ix) => ix !== i)); }}>×</Button>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="grid sm:grid-cols-2 gap-2">
                {positions.map((p) => (
                  <div key={p.id} className="flex items-center justify-between border rounded-xl p-2">
                    <Input className="h-8" value={p.name} onChange={() => { /* Position names are edited inline in main grid */ }} />
                    <Button variant="ghost" size="sm" onClick={() => removePosition(p.id)}>Remove</Button>
                  </div>
                ))}
              </div>
            </div>
